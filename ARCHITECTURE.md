...
| `shape.line` | `points[]`, `stroke`, `strokeWidth` | bbox + local points | Points are local to `x`/`y`, so resize is a pure scale. Ends may bind to a box (`start`/`end` anchors, R14); stored points stay authoritative. |
| `shape.arrow` | line + `pointerLength`, `pointerWidth` | bbox + local points | Arrowhead is geometry, not a texture. Shares the line's optional endpoint bindings. |
...
| **v0.2.2** | Deliberate mouse-wheel zoom (`wheelZoomFactor` + `WheelEvent.deltaMode` normalization), custom stroke/fill colour pickers (`change`-event only: one pick = one undo step), baseline application (R13): `diffScenes` routes a whole document through the delta path; editor `getBaseline`/`applyBaseline`/`loadBaseline`/`downloadBaseline` + `EditorOptions.scene`; viewer `loadBaseline`/`applyBaseline`; chrome save/load-baseline buttons; chrome keys 44→48; `localeDirection` made CLDR-version-stable (explicit script subtag beats `Intl` `textInfo`) | **shipped** |
| **v0.2.3** | Line styles (solid/dashed/dash-dot; `strokeStyle` on every stroked object, missing means solid — no document-shape change), whiteboard/blackboard switch (`BOARD_THEMES` presets; page background + grid ride the view-config rule so the PNG export follows and the document stays byte-identical; chrome flips through the inline theme layer + `[data-board]` stylesheet, custom-picker glyph frames included; `boardTheme`/`onBoardThemeChange` chrome options), picker-icon polish (transparency checkerboard, neutral fill samples, framed custom-stroke/custom-fill glyphs); chrome keys 48→54 | **shipped** |
| **v0.2.4** | Text typography menus: font-family (four generic system stacks, each option rendered in its own stack) and font-size selects preset the next text and restyle the selection with box re-measure in one undo step; custom values surface as honest raw-labelled options (custom-swatch precedent); `FONT_FAMILIES`/`FONT_SIZES` in `@coslate/konva`; chrome keys 54→60; README header screenshot | **shipped** |
| **v0.2.5** | Connector endpoint binding (R14, the flowchart/sequence-diagram foundation): line/arrow ends snap-bind to rect/ellipse/text boxes (8 screen px) and follow them through move/resize/rotate/text re-measure — transient node tracking mid-gesture, same-transaction `points` re-derivation at commit (one gesture = one undo), delete strips bindings with undo restoring them; `EndpointBinding` on `LineData` is optional metadata and `points` stay authoritative, so renderers/export/viewer/sync are untouched and the wire format gains no version bump | **shipped** |
